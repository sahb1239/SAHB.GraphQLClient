...
{
    public class ValidationHelloListNonNull : GenericHelloTest<ValidationHelloListNonNull.GraphQLQuery, ValidationHelloListNonNull.TestHelloQuery, ValidationHelloListNonNull.TestInvalidHelloQuery>
    {
        public ValidationHelloListNonNull(GraphQLWebApplicationFactory<GenericQuerySchema<GraphQLQuery>> factory) : base(factory)
        {
        }
        ...
        public async Task Hello_Has_Correct_Type()
        {
            ...
            Assert.Equal(GraphQLTypeKind.NonNull, helloType.Type.OfType.Kind);
            Assert.Equal("String", helloType.Type.OfType.OfType.Name);
            Assert.Equal(GraphQLTypeKind.Scalar, helloType.Type.OfType.OfType.Kind);
        }
        ...
        public class GraphQLQuery : ObjectGraphType
        {
            public GraphQLQuery()
            {
                Field<ListGraphType<NonNullGraphType<StringGraphType>>>("hello", resolve: context => new[] { "query" });
            }
        }
